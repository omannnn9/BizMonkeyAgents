...
- **[Architecture](./ARCHITECTURE.md)** — stack, the no-login/service-role
  design, request flows, app structure, observability.
- **[Data model](./DATA_MODEL.md)** — every table across all 11 migrations,
...
- **[API reference](./API_REFERENCE.md)** — every route under `app/api/`:
  method, params, request/response shape.
- **[Frontend](./FRONTEND.md)** — every page and shared component,
  ...
  treatment.
- **[Testing](./TESTING.md)** — the scripted checks that need a live
  Supabase project + real Groq/Voyage keys, and why the old demo-mode e2e
  suite was retired.
